...
y_pred_const = np.ones(len(y)) * y.mean()
...
y_pred_const = np.ones(len(y_test)) * y.mean()
print(f"Ошибка константного предсказания на новых данных: {mean_squared_error(y_pred_const, y_test)}")
